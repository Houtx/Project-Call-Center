...
Optional usage telemetry is compile-time disabled unless an HTTPS endpoint is configured. Configured builds enable it by default at runtime, retain an explicit opt-out, and never block calling. Routine activity is sent as a small UTC-day aggregate using a random telemetry identifier unrelated to online device/session identifiers. On the first outbound call of each local day, an application-scoped task requests GPS and network providers without delaying call launch. It stores one latitude/longitude snapshot with accuracy and capture time, retries later calls with a bounded policy when location fails, and triggers a revision-aware upload when it succeeds. A separate local SQLite store deduplicates settled attempts and queues no more than 31 dirty daily metric and location rows. The receiver keeps location detail only for the configured detail-retention window; the administrator map chooses the latest device location in the selected period, clusters at low zoom, and returns pseudonymous device points at high zoom. Payloads exclude phone numbers, names, SIM data, server addresses, filenames, customer IDs and call-level timestamps.
