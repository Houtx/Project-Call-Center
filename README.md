...
- “离线模式”表示外呼业务不依赖公司服务器；APP 检查版本更新、读取公司公告，以及已启用的匿名使用统计仍可能访问互联网。
...
`call.haoyunqiankun.com` 首页同时提供独立的运营统计和公告管理端。配置统计地址的公司内部 APP 默认开启匿名统计；`0.7.2` 会把旧版本中已经关闭的统计重新开启一次，之后关闭时需要输入手机当天日期组成的 8 位管理口令（`yyyyMMdd`）。日常上报包含匿名活跃、版本、模式、国家/时区和外呼数量汇总；每天首次外呼会在不阻塞拨号的前提下采集一次设备位置，用于管理端区域聚合和设备分布地图，不记录连续轨迹。管理员可发布、编辑和删除终端公告；APP 冷启动会显示未读公告并要求前台阅读至少 10 秒，当前公告修改后会按新修订再次显示。该服务不接收号码、姓名、坐席、SIM、导入文件或逐通记录，统计 IP 仅保留 HMAC 和脱敏网段。
...
- 独立模式的客户与通话数据仅保存在手机本机，不会上传到 CRM；配置统计地址的公司内部构建默认开启匿名使用统计，关闭需要当天 8 位管理口令，统计不包含号码、姓名、SIM 或服务器信息。
